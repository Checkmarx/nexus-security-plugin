...
@Named
public class NugetScanner {
	private static final Logger LOG = LoggerFactory.getLogger(NugetScanner.class);
	private static AssetStore assetStore = null;

	@Inject
	public NugetScanner(final AssetStore assetStore) {
		NugetScanner.assetStore = assetStore;
	}

	public static ScanResult scan(@Nonnull Context context, Payload payload, CheckmarxClient CheckmarxClient) {
		if (payload == null) {
			return null;
		}
		...
		if (payload instanceof Content) {
			...
			NestedAttributesMap nugetAttributes = asset.attributes().child("nuget");
			Object nameAttribute = nugetAttributes.get("name");
			packageName = nameAttribute != null ? nameAttribute.toString() : "";
			Object versionAttribute = nugetAttributes.get("version");
		...
		if (checkmarxPropertiesExist(payload)) {
			LOG.debug("nuget artifact {}:{} was already scanned. Skip scanning", packageName, packageVersion);
			...
			try {
				Package nugetPackage = new Package(PackageType.NUGET.getType(), packageName, packageVersion);
				...
				if (response.isSuccessful() && response.body() != null) {
					...
					LOG.debug("testnuget response: {}", responseAsText);
				}
				...
				LOG.error("Cloud not test nuget artifact: {}", context.getRequest().getPath(), ex);
			} catch (Exception e) {
	...
	private static boolean checkmarxPropertiesExist(Payload payload) {
		NestedAttributesMap checkmarxSecurityMap = getCheckmarxSecurityAttributes(payload);
	...
	private static void updateAssetAttributes(@Nonnull TestResult testResult, @Nonnull String packageName, @Nonnull String packageVersion, Payload payload) {
		if (payload instanceof Content) {
			Asset asset = ((Content) payload).getAttributes().get(Asset.class);
			if (asset == null) {
				return;
	...
	private static NestedAttributesMap getCheckmarxSecurityAttributes(Payload payload) {
		if (!(payload instanceof Content)) {
			return null;
		}
		Asset asset = ((Content) payload).getAttributes().get(Asset.class);
